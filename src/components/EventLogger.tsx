  ...
  const WithEventLogging: React.FC<P> = (props) => {
    ...
    const wrapClickHandler = useCallback((handler?: MouseEventHandler) => {
      return (event: React.MouseEvent) => {
        const logMessage = createLogMessageWithLogType(createLogMessage('click', event.target as HTMLElement));
    ...
    const wrapChangeHandler = useCallback((handler?: ChangeEventHandler) => {
      return (event: React.ChangeEvent<HTMLInputElement>) => {
        const logMessage = createLogMessageWithLogType(createLogMessage('change', event.target, event.target.value));
        // messageContext?.setMessages(prev => [...prev, logMessage]);
        sendLog(logMessage);
        handler?.(event);
      };
