...
export default function Screen1Step1({ sendAdminMessage }: BaseProps) {
  ...
  const level1 = {
    n: gameStateRef.current.state1.key.n,
    a: gameStateRef.current.state1.key.a,
    b: gameStateRef.current.state1.key.b
  }
  const level3 = {
  ...
  return (
    <div className="mx-auto min-h-screen overflow-hidden relative" style={{ backgroundImage: `url(${background.src})`, backgroundSize: '100% 100%', backgroundRepeat: 'no-repeat'}}>
      ...
      <div className='flex justify-center items-center space-x-8 text-2xl leading-none py-24 relative'>
        ...
        <div className='flex flex-col items-center'> 
          <h1 className='absolute top-[50px] '>Level 1</h1>
          <NewKey n={level1.n} a={level1.a} b={level1.b} isActive={true}/>
        </div>
        <div className='flex flex-col items-center'> 
          <h1 className='absolute top-[84px] text-xl'>Level 2</h1>
          <NewKey n={level2.n} a={level2.a} b={level2.b} isActive={false}/>
        </div>
        <div>
