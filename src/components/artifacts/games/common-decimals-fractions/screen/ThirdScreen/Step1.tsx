  ...
  return (
    <div className="mx-auto min-h-screen overflow-hidden" style={{ backgroundImage: `url(${background.src})`, backgroundSize: '100% 100%', backgroundRepeat: 'no-repeat' }}>
      ...
      <div className='absolute flex items-center justify-center w-full gap-[3vh] translate-y-[5vh]'>
        ...
        <div className='flex flex-col items-center justify-center'>
          <h1 className='-translate-y-[2vh] text-[3vh]'>Level {gameStateRef.current.state3.key.level}</h1>
          <NewKey  n={gameStateRef.current.state3.key.numerator} a={gameStateRef.current.state3.key.denominator_1} b={gameStateRef.current.state3.key.denominator_2} isActive={true} />
        </div>
        <div className='flex flex-col items-center justify-center'>
          <h1 className='translate-y-[4vh] text-[2.5vh]'>Level {gameStateRef.current.state4.key.level}</h1>
          <NewKey  n={gameStateRef.current.state4.key.numerator} a={gameStateRef.current.state4.key.denominator_1} b={gameStateRef.current.state4.key.denominator_2} isActive={false} />
        </div>
