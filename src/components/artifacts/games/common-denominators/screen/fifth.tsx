...
export default function FifthScreen({ sendAdminMessage }: BaseProps) {
  ...
  useEffect(() => {
    if (parseInt(lcdInputs.result) === lcd) {
      goToStep('fifth', setGameStateRef, 2);
    }
  }, [lcdInputs]);
                ...
                <div className="flex gap-2">
                  {lcdInputs.multiples1.map((value, index) => (
                    <div className="flex flex-col items-center" key={`mult1-${index}`}>
                      ...
                      />
                      <span className="text-sm text-gray-500">{fraction1.denominator} x {index + 1}</span>
                    </div>
                  ))}
                </div>
              </div>
                  ...
                  {lcdInputs.multiples2.map((value, index) => (
                    <div className="flex flex-col items-center" key={`mult2-${index}`}>
                      <Input
                        ...
                        className="w-12 h-12 border-2 border-gray-300 rounded text-center"
                      />
                      <span className="text-sm text-gray-500">{fraction2.denominator} x {index + 1}</span>
                    </div>
                  ))}
                </div>
              </div>
