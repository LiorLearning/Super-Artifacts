...
export default function HintPopup({ number, setHintPopup, sendAdminMessage }: HintPopupProps) {
  ...
  const handleOnesCorrect = () => {
    ...
    if(narrations.Screen2Step0Message6.send) {
      sendAdminMessage("agent", formatMessage(narrations.Screen2Step0Message6.content, { number }));
    }
  };
            ...
            <div className={`flex flex-col items-center justify-center gap-[2vh]`}>
              <div className="flex flex-wrap gap-[1vh] w-[24vh]">
                {Array(Math.floor(number / 10)).fill(0).map((_, i) => (
                  <span key={i} className="bg-[#f8a34b] rounded-[1vh] w-full h-[4vh] inline-block shadow-[0.2vh_0.2vh_0_0_#393f3f]"></span>
                ))}
              </div>
              <div className="flex items-center justify-center text-[3vh]">
                <NewInput
                  className="border-2 border-black rounded-[1vh] w-[5vh] h-[5vh] flex items-center justify-center mr-[1vh] text-center"
                  value={tensInput}
                  onValueChange={setTensInput}
            ...
            <div className={`flex flex-col items-center justify-center gap-[2vh]`}>
              <div className="flex flex-wrap gap-[1vh] w-[20vh]">
                {Array(number % 10).fill(0).map((_, i) => (
                  <span key={i} className="bg-[#5cdbec] rounded-[1vh] w-[4vh] h-[4vh] inline-block shadow-[0.2vh_0.2vh_0_0_#393f3f]"></span>
                ))}
              </div>
              <div className="flex items-center justify-center text-[3vh]">
                <NewInput
