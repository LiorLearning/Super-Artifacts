...
export default function Screen2Step2({ sendAdminMessage }: BaseProps) {
  ...
  function generateArray(center: number, type: string): string[] {
    const start = center - 1;
    if (type === 'numerator') {
      return Array.from({ length: 3 }, (_, i) => (start + i).toString());
    } else {
      const arr = Array.from({ length: 2 }, (_, i) => (start + i).toString());
      arr.push((center * whole).toString());
      return arr;
    }
  }
  ...
  const onIncorrect = (attempt: string, correctAnswer: string, inputType: string, wholeNumber?: string) => {
    ...
    switch (inputType) {
      ...
      case 'numerator':
        ...
        break;
      case 'denominator':
        sendAdminMessage('admin', `User answered incorrectly for the denominator, correct answer is ${fraction.denominator} but user answered ${attempt}, Help user solve the problem. Diagnose socratically.`);
        break;
    }
  };
  ...
  const onCorrect = (example: 'first' | 'second') => {
    if (example === 'first') {
      sendAdminMessage('agent', `Great! You've completed the first example! Let's try one more!`);
    } else {
  ...
  if (selectedFraction.denominator === (fraction.denominator * whole).toString()) {
    setTimeout(() => {
  ...
  return (
    <>
      {selectedFraction.denominator === (fraction.denominator * whole).toString() && showWrongPage ?
        <WrongPageProps level={2} whole={whole} numerator={fraction.numerator} denominator={fraction.denominator} sendAdminMessage={sendAdminMessage} setSelectedFraction={setSelectedFraction} setShowWrongPage={setShowWrongPage} />
        :
        <div className="flex flex-col min-h-screen">
          ...
          <div className="my-8 bg-[#fff0e5] p-6 py-8 relative">
            <div className="display flex justify-center items-center gap-4 text-black text-2xl leading-none mx-auto my-4">
              ...
              <div className="w-[50%] flex flex-col gap-4 ml-2 justify-center items-center">
                ...
                <div
                  ...
                  className={`bg-[#b7611c] text-white py-3 px-6 leading-none text-2xl text-center shadow-[-3px_3px_0px_0px_rgba(0,0,0)] w-[100%] shadow-black 
              ...
              <div className={`flex flex-col items-center justify-center gap-2 ${!showDropDown ? 'opacity-50' : ''}`}>
                <DropDown options={generateArray(fraction.numerator * whole, 'numerator')} selected={selectedFraction.numerator} showDropDown={showDropDown} correctValue={fraction.numerator * whole + ""} onSelect={(selected) => {
                  setSelectedFraction((prev) => ({ ...prev, numerator: selected }));
                }}
                  onIncorrect={(current) => {
                    sendAdminMessage('admin', `User answered incorrectly for the numerator, correct answer is ${fraction.numerator * whole} but user answered ${current}, Help user solve the problem. Diagnose socratically.`);
                  }}
                  onCorrect={() => {
                ...
                <DropDown options={generateArray(fraction.denominator, 'denominator')} selected={selectedFraction.denominator} correctValue={fraction.denominator + ""} showDropDown={showDropDown} onSelect={(selected) => {
                  ...
                  onIncorrect={(current) => {
                    console.log(current, fraction.denominator * whole);
                    if (current === (fraction.denominator * whole).toString()) {
                      sendAdminMessage('agent', `Let's find out if ${fraction.numerator * whole}/${current} is correct or not!`);
                    } else {
                      sendAdminMessage('admin', `User answered incorrectly for the denominator, correct answer is ${fraction.denominator} but user answered ${current}, Help user solve the problem. Diagnose socratically.`);
                    }
                  }}
                />
              </div>
