...
const GhostBuster = () => {
  ...
  const [gameState, setGameState] = useState<any>('ready');
  const [shake, setShake] = useState<any>(false);
  const [currentLocation, setCurrentLocation] = useState<any>(0);
  const [story, setStory] = useState<any>('');
  const [foundFactors, setFoundFactors] = useState<any>([]);
  const [hintsRemaining, setHintsRemaining] = useState<any>(3);
  const [showHint, setShowHint] = useState<any>(false);

  const generateStory = (locationIndex: any, level: any) => {
    const stories: any = [
  ...
  const generateGhosts = () => {
    const number: any = Math.floor(Math.random() * 61) + 40;
    const factors: any = Array.from(Array(number + 1), (_, i) => i)
      .filter((i: any) => number % i === 0);
  ...
  const checkAnswer = () => {
    const userNumber: any = parseInt(answer.trim());
    if (isNaN(userNumber)) {
    ...
    if (question.answer.includes(userNumber) && !foundFactors.includes(userNumber)) {
      ...
      setGhosts((prev: any) => {
        const newGhosts: any = [...prev];
        const firstAlive: any = newGhosts.findIndex((g: any) => !g.defeated);
        if (firstAlive !== -1) {
  ...
  const useHint = () => {
    if (hintsRemaining > 0) {
      setHintsRemaining((prev: any) => prev - 1);
      setShowHint(true);
  ...
  const playSound = (type: any) => {
    ...
    audio.play().catch((e: any) => console.log('Audio play failed:', e));
  };
        ...
        {gameState === 'playing' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-5 gap-4 mb-6">
              {ghosts.map((ghost: any, index: any) => (
                <div 
                  key={index}
